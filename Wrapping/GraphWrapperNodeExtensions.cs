...
public static class GraphWrapperNodeExtensions
{
    ...
    public static T? SingularReverse<T>(this GraphWrapperNode @object, INode predicate, ValueMapping<T> map, bool throwWhenMissing = false, bool throwWhenMore = false)
    {
        using var subjects = @object.Graph
            .GetTriplesWithPredicateObject(predicate, @object)
            .Select(triple => triple.Subject)
            .In(@object.Graph)
            .GetEnumerator();

        if (!subjects.MoveNext())
        {
            if (throwWhenMissing)
            {
                throw new InvalidOperationException("less than one");
    ...
    public static ISet<T> Subjects<T>(this GraphWrapperNode @object, INode predicate, NodeMapping<T> nmap, ValueMapping<T> vmap) =>
        new NodeSet<T>(@object, predicate, TripleSegment.Object, nmap, vmap);
